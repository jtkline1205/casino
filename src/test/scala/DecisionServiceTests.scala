...
class DecisionServiceTests {
	...
	val weakDealerCards = Array(new Card(Rank.TWO), new Card(Rank.THREE), new Card(Rank.FOUR),
			new Card(Rank.FIVE), new Card(Rank.SIX))

	val strongDealerCards = Array(new Card(Rank.SEVEN), new Card(Rank.EIGHT), new Card(Rank.NINE),
			new Card(Rank.TEN), new Card(Rank.ACE))

	def createHand(value1: Int, value2: Int) {
		HandFactory.createHand(value1, value2)
	}

	val decisionService = new DecisionService()

	@Test
	def testStandOnStrongHandsVersusAllDealerCards() {
		assertEquals(1, 1)
